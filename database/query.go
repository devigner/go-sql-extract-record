...
// PrintResult Write the result to the stdout
...
// SliceIndex Get the index of a needle
...
// SelectFromDB execute an selection query
func SelectFromDB(table string, field string, value string, depth int, path []string) {
	...
	rows, err := ConnectionTargetSchema.Queryx(query)
	if err != nil {
